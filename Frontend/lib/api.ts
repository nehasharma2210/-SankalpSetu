...
export const API_CONFIG = {
  ...
  AI_MODEL_URL: process.env.AI_MODEL_URL || "your-ai-model-endpoint",
  DATABASE_URL: process.env.DATABASE_URL || "postgresql://localhost:5432/sankalp_setu",
  RECOMMENDER_MODEL_URL: process.env.RECOMMENDER_MODEL_URL || "http://localhost:5000",
...
static async getSchemeMatches(userId: string) {
  const response = await fetch(`${API_CONFIG.BASE_URL}/schemes/matches/${userId}`);
  if (!response.ok) throw new Error("Failed to fetch scheme matches");
  return response.json();
}
static async getUserProgress(userId: string) {
  const response = await fetch(`${API_CONFIG.BASE_URL}/learning/progress/${userId}`);
  if (!response.ok) throw new Error("Failed to fetch user progress");
  return response.json();
}
  static async saveChatMessage(chatData: {
    userId: string;
    message: string;
    response: string;
    context?: any;
